...
void BLE_Process(void) {
  ...
  if (APP_FLAG(CONNECTED)) {
    ...
    ret = aci_gatt_read_handle_value(
        input_report_char_handle + 2, 0, sizeNotifications_enabled_val,
...
uint8_t hidSetDeviceDiscoverable(uint8_t mode, uint8_t nameLen, uint8_t *name) {
  ...
  if (mode == LIMITED_DISCOVERABLE_MODE) {
    ...
    ret = aci_gap_set_limited_discoverable(
        ADV_IND, (ADV_INTERVAL_MIN_MS * 1000) / 625,
        (ADV_INTERVAL_MAX_MS * 1000) / 625, PUBLIC_ADDR, NO_WHITE_LIST_USE,
