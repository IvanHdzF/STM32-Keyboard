...
uint8_t hidSetDeviceSecurity(uint8_t MITM_Mode, uint8_t fixedPinUsed,
                             uint32_t fixedPinValue) {
  if (!MITM_Mode) {
    return -1;
  }
  uint8_t ret;
  ret = aci_gap_set_authentication_requirement(
      BONDING, MITM_PROTECTION_REQUIRED,
      // MITM_PROTECTION_NOT_REQUIRED,
      SC_IS_SUPPORTED,
      // SC_IS_NOT_SUPPORTED,
      // KEYPRESS_IS_SUPPORTED,
      KEYPRESS_IS_NOT_SUPPORTED, 7, 16, fixedPinUsed, fixedPinValue, 0x00);
  if (ret != BLE_STATUS_SUCCESS) {
...
/**
 * @brief This function sets public Address, Initializes Gatt and Gap Services
 * according to HID BLE Keyboard specs.
 * @param IO_Capability set IO Capability
 * @param connParam Update values for Preferred Connection Parameters
 * Characteristics
...
uint8_t hidDevice_Init(uint8_t IO_Capability, connParam_Type connParam,
  ...
  aci_hal_read_config_data(CONFIG_DATA_PUBADDR_OFFSET, &valueLen,
                           (uint8_t *)valueRead);
  ...
  aci_hal_read_config_data(CONFIG_DATA_DIV_OFFSET, &valueLen,
                           (uint8_t *)valueRead);
  ...
  aci_hal_read_config_data(CONFIG_DATA_ER_OFFSET, &valueLen,
                           (uint8_t *)valueRead);
  ...
  aci_hal_read_config_data(CONFIG_DATA_IR_OFFSET, &valueLen,
                           (uint8_t *)valueRead);
  ...
  aci_hal_read_config_data(CONFIG_DATA_LL_WITHOUT_HOST, &valueLen,
                           (uint8_t *)valueRead);
  ...
  ret = aci_gap_init(GAP_CENTRAL_ROLE | GAP_PERIPHERAL_ROLE, 0x0, 0x20,
                     &service_handle, &dev_name_char_handle,
                     &appearance_char_handle);
  aci_gatt_update_char_value(service_handle, appearance_char_handle, 0, 2,
                             appareance);

  ret =
      aci_gatt_update_char_value_ext(0, service_handle, dev_name_char_handle, 0,
                                     dev_name_len, 0, dev_name_len, dev_name);
  ret = aci_gatt_update_char_value_ext(
      0, service_handle, appearance_char_handle + 2, 0, sizeof(connParam), 0,
      sizeof(connParam), (uint8_t *)&connParam);
