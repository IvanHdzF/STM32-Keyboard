...
void APP_UserEvtRx(void *pData) {
  ...
  if (hci_pckt->type == HCI_EVENT_PKT) {
    ...
    if (event_pckt->evt == EVT_LE_META_EVENT) {
      ...
      for (i = 0; i < (sizeof(hci_le_meta_events_table)) /
    ...
    // process vendor event
      ...
      for (i = 0; i < (sizeof(hci_vendor_specific_events_table) /
...
void aci_gap_pairing_complete_event(uint16_t Connection_Handle, uint8_t Status,
  ...
  if (Status != BLE_STATUS_SUCCESS) { /* Pairing Failed */
    BLUENRG_PRINTF("________aci_gap_pairing_complete_event FAILED:0x%02x with "
                   "reason 0x%02x\r\n",
                   Status, Reason);
  } else {
    BLUENRG_PRINTF(
        "________aci_gap_pairing_complete_event with status 0x%02x\r\n",
        Status);
  }
}
...
void hci_le_connection_update_complete_event(uint8_t  Status,
  ...
  BLUENRG_PRINTF(
      "____________hci_le_connection_update_complete_event Status: %d\r\n",
      Status);
  BLUENRG_PRINTF("____________hci_le_connection_update_complete_event "
                 "Connection_Handle: %d\r\n",
                 Connection_Handle);
  BLUENRG_PRINTF("____________hci_le_connection_update_complete_event "
                 "Conn_Interval: %d\r\n",
