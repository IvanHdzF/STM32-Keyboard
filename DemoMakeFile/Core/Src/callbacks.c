                                    ...
                                    uint8_t Reason)
{
  ...
  if (Status != BLE_STATUS_SUCCESS) { /* Pairing Failed */
    BLUENRG_PRINTF("________aci_gap_pairing_complete_event FAILED:0x%02x with reason 0x%02x\r\n", Status, Reason);
  }
  else {
    BLUENRG_PRINTF("________aci_gap_pairing_complete_event with status 0x%02x\r\n", Status);
  }
}
                                    ...
                                    uint16_t Offset)
{
  ...
  ret = aci_gatt_allow_read(connection_handle);
    ...
    {
      BLUENRG_PRINTF("aci_gatt_allow_read() failed: 0x%02x\r\n", ret);
    }
}
                                             ...
                                             uint16_t Supervision_Timeout)
{
  ...
  BLUENRG_PRINTF("____________hci_le_connection_update_complete_event Status: %d\r\n",Status);
  BLUENRG_PRINTF("____________hci_le_connection_update_complete_event Connection_Handle: %d\r\n",Connection_Handle);
  BLUENRG_PRINTF("____________hci_le_connection_update_complete_event Conn_Interval: %d\r\n",Conn_Interval);
  BLUENRG_PRINTF("____________hci_le_connection_update_complete_event Conn_Latency: %d\r\n",Conn_Latency);
  BLUENRG_PRINTF("____________hci_le_connection_update_complete_event Conn_Latency: %d\r\n",Supervision_Timeout);
}
